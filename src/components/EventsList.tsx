...
export function EventsList({ events }: EventListProps) {
  ...
  const getNextWednesdayEvent = (): Event | null => {
    ...
    if (nextWednesday > new Date()) {
      return {
        ...
        description: "Ankara Lindy Hop haftalık dans buluşması. Yeni başlayanlar ve tüm seviyeler davetlidir!",
    ...
    >
      ...
      <div className="flex-1 p-4 sm:p-6">
        <h2 className="mb-2 text-xl font-semibold text-zinc-100 sm:text-2xl">
          {event.title}
          {event.isRecurring && (
            <span className="ml-2 text-sm text-orange-300">(Haftalık Etkinlik)</span>
          )}
        </h2>
  ...
  const upcomingEvents = nextWednesdayEvent
    ? [...events.filter(event => new Date(event.date) > new Date()), nextWednesdayEvent]
    : events.filter(event => new Date(event.date) > new Date());

  return (
    <div className="mx-auto max-w-4xl px-4 py-8">
      <h1 className="mb-8 text-center text-2xl font-bold text-zinc-100 sm:text-3xl">
